...
def report_machine_data(static_data: str | None = None, dynamic_data: str | None = None):
	...
	# Обробляємо мережевих сусідів (discovery)
	...
	try:
		...
		for neighbor in neighbors:
			...
			if not neighbor_asset:
				# Резервний пошук в загальному полі актива (для роутерів)
			...
			if neighbor_asset and neighbor_asset != asset_name:
				...
				# Порт на нашому активі (унікальний для кожного сусіда)
				my_port_name = f"Auto-{iface}-{neighbor_mac[-8:]}"

				# Шукаємо наш вихідний порт
				my_port = frappe.get_doc(
					"oiNetworkPort", {"asset": asset_name, "mac_address": ("is", "set")}
				)  # Спрощено, в ідеалі шукаємо той що в тій же підмережі
				# Для простоти поки створюємо новий Discovery порт на нашому боці якщо його немає
				...
				# Цільовий порт на активі сусіда
