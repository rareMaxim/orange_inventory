...
let current_owner_fiter = function (frm) {
	frm.fields_dict["items"].grid.get_field("asset").get_query = function (doc, cdt, cdn) {
		var child = locals[cdt][cdn];
		//console.log(child);
		if (frm.doc.from_organization_filter) {
			return {
				filters: [["current_owner", "=", frm.doc.from_organization]],
			};
		} else {
			return {
				filters: [],
...
frappe.ui.form.on("oiIssueOrderItem", {
	...
	asset: function (frm, cdt, cdn) {
		...
		if (row.asset) {
			frappe.db.get_value("oiAsset", row.asset, ["quantity", "cost"]).then((r) => {
				if (r) {
...
function update_all_rows(frm) {
	...
	const promises = frm.doc.items.map((item) => {
		if (item.asset) {
			return frappe.db.get_value("oiAsset", item.asset, "quantity").then((r) => {
