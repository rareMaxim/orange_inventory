    ...
    let d = new frappe.ui.Dialog({
        ...
        fields: [
            {
                ...
                reqd: 1,
                description: __("Введіть  <b>{0}</b> серійних номерів, кожен з нового рядка або через кому.", [frm.doc.quantity])
            }
        ],
        ...
        primary_action(values) {
            ...
            if (serials.length !== frm.doc.quantity) {
                frappe.msgprint(__("Кількість введених серійних номерів ({0}) не співпадає з кількістю активу ({1}).", [serials.length, frm.doc.quantity]));
                return;
            }
            ...
            frappe.call({
                ...
                callback: function (r) {
                    if (!r.exc) {
                        frappe.show_alert({ message: __("Актив успішно розділено"), indicator: "green" });
                        frm.reload_doc();
                        d.hide();
                    }
                }
